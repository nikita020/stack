...
import { coinColors, windowHeight, colorViolet3 } from '../../../../constants'
...
export default class PortfolioCustomSetUp extends React.Component {
  ...
  renderRow () {
    return (
      <View style={ {marginBottom: windowHeight * 0.05} }>
        {
          listCoins.map((l, i) => (
            <View key={i} style={styles.coinsItemContainer}>
  ...
  render () {
    ...
    if (isNaN((this.state.customBTC) / (this.state.customBTC + this.state.customETH + this.state.customXRP + this.state.customBCH)) === true) {
      ...
      percentXRP = (this.state.customLTC) / (this.state.customBTC + this.state.customETH + this.state.customXRP + this.state.customBCH) * 100
      percentBCH = (this.state.customNEO) / (this.state.customBTC + this.state.customETH + this.state.customXRP + this.state.customBCH) * 100
    }
    ...
    return (
      <ScrollContainer>
        <View style={ [styles.containerCenter, {marginBottom: constants.windowWidth * 0.35, position: 'relative'}] }>
          <View style={[styles.vw100, {backgroundColor: constants.headerColorViolet, height: 130, position: 'absolute', top: 0, left: 0}]}/>
